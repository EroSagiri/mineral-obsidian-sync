 ...
 * response at all. Obsidian on Android throws exactly that for a HEAD request whose response is a
 * 404 with no body — which is why every error-path probe in the matrix exists.
 */
...
export function describeTransportError(error: unknown): string {
  return error instanceof Error ? error.message.replace(/\s+/g, " ").slice(0, 160) : "unknown error";
}
